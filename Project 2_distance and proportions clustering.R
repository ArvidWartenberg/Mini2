{# <- brackets around whole program
  ...
  }
  {
    ...
    colnames(errorMat) <- c("K-means","K-means 50","Bottom Up Average","Bottom Up Ward's","GMM","Density")
  }
  
  
  for(i in 1:maxItr){
    
    iterationTime <- proc.time() # Time iteration
    ...
    for(batch in 1:nBatches){  
      ...
      muC2 <- rep(0,2*nDim); sigmaC2 <- 10; muC2[1] <- dist; sigmaMatC2 <- sigmaC2*diag(2*nDim);
      ...
      # # ------ K-means 50 start------
      km <- kmeans(x=X[,-1] , centers=2, iter.max = 100, nstart = 50,
                   algorithm = c("Lloyd"), trace=FALSE)
      ...
      print("Bottom Up Ward's done")
      
      #------ Gaussian Mixture Models ------
      EMCVal<-.EMControl(short.iter=50)
      emobj <- init.EM(X[,-1],stable.solution = TRUE,EMC=EMCVal,nclass=2,method="Rnd.EM")
      gmm <- emcluster(X[,-1], emobj, assign.class = TRUE)
      clusters <- cbind(gmm$class,X[,-1])
      
      predicted <- factor(gmm$class)
      actual <- X[,1]
      
      #rm(emobj,gmm,clusters)
      # ------ Evaluation ------
      ...
      db <- dbscan::dbscan(X[,-1], eps = 0.7, minPts = 5)
    ...
    errorMat[i,] <- errorMat[i,]/nBatches   # Average over crossvalidation folds
    iterationTime <- proc.time() - iterationTime
    iterationTime <- iterationTime/60
    print(paste('Current iteration: ',i,'Min left: ', format(round(iterationTime[3]*(maxItr-i), 2), nsmall = 2)))
